...
contract('AlyraERC20Token', accounts => {
  ...
  it('check balance after transfer', async () => {
    ...
    assert.equal(balanceOwnerAfterTransfer.toNumber(), balanceOwnerBeforeTransfer.sub(BN(amount)).toNumber());
    assert.equal(balanceRecipientAfterTransfer.toNumber(), balanceRecipientBeforeTransfer.add(BN(amount)).toNumber());
  ...
  it('check transferFrom after approve', async () => {
    ...
    const balanceRecipientBeforeTransferFrom = await instance.balanceOf(_recipient);
    ...
    await instance.transferFrom(_owner, _recipient, amount, { from: _recipient });
    
    const balanceOwnerAfterTransferFrom = await instance.balanceOf(_owner);
    const balanceRecipientAfterTransferFrom = await instance.balanceOf(_recipient);
