...
import { CLEAR_CREATE_VOTING_SESSION_TX_ERROR, LOAD_VOTING_CONTRACT_INFO } from '../actions';
...
export default ({ children }: IWalletContainerWrapperProps) => {
  ...
  const { contract, error, txPending } = useSelector((state: RootState) => state.voting);
  ...
  return <>
    <Container sx={{ mt: 3, minHeight: '1024px' }} maxWidth="xl">
      ...
      <Snackbar open={error != null} onClose={handleCloseErrorSnack}>
        <Alert severity="error" sx={{ width: '100%', color: 'red' }} onClose={handleCloseErrorSnack}>
          {error}
        </Alert>
      </Snackbar>
      ...
      <Snackbar open={txPending}>
        <Alert severity="info" sx={{ width: '100%' }}>
          <CircularProgress size={16} disableShrink thickness={3}/>Transaction pending
        </Alert>
      </Snackbar>
    </Container>
