                        ...
                        <Item>
                            ...
                            <List sx={{ width: '100%', maxWidth: 450, bgcolor: 'background.paper' }}>
                                {_.map(result.data.winningProposals, (proposal, idx) => {
                                    return <ListItem key={`winning_proposal_${idx}`}>
                                        <ListItemAvatar>
                                            <AddressAvatar address={proposal.proposer}/>
                                        </ListItemAvatar>
                                        <ListItemText primary={proposal.description} secondary={`Nb votes: ${proposal.voteCount}`} />
                                    </ListItem>
                                })}
                            </List>
                        </Item>
                    ...
                    <Grid item xs={12} md={6}>
                        <Item>
                            ...
                            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                ...
                                <div style={{ color: theme.palette.text.secondary }}>Nb voters</div>
                                <div style={{ fontSize: '2em' }}>{result.data.votersCount}</div>
                                
                                <div style={{ color: theme.palette.text.secondary }}>Nb votes</div>
                                <div style={{ fontSize: '2em' }}>{result.data.totalVotes}</div>
                                
                                <div style={{ color: theme.palette.text.secondary }}>Abstention</div>
                                <div style={{ fontSize: '2em' }}>{result.data.abstention}</div>

                                <div style={{ color: theme.palette.text.secondary }}>Blank votes</div>
