...
import { Alert, AlertColor, Box, Button, Grid, IconButton, Step, StepLabel, Stepper, Tooltip, Typography, useTheme } from '@mui/material';
...
import { MAX_PROPOSALS, NEXT_VOTING_SESSION_STEP, VotingSessionWorkflowStatus } from '../../actions';
...
export default () => {
    ...
    const { contract, currentSession, voters } = useSelector((state: RootState) => state.voting);
    const { account } = useSelector((state: RootState) => state.ethNetwork);

    useEffect(() => {
        const _nbVotes = _.reduce(voters.items, (acc, v) => {
            if (!v.hasVoted) acc++;
            return acc;
        }, 0);
    ...
    const renderVotingSessionAlert = () => {
        ...
        const renderAlert = (message: string, color: AlertColor) => {
            return <Alert variant="outlined" severity={color} action={contract.info.isOwner && <Button
                    color={color}
                    onClick={() => {
                        dispatch(NEXT_VOTING_SESSION_STEP.request({ sessionId: currentSession.item.id, currentStatus: currentSession.item.status }));
                    }}
                >
        ...
        if (contract.info.isOwner) {
            ...
            if (currentSession.item.status === VotingSessionWorkflowStatus.ProposalsRegistrationStarted) {
                return renderAlert('Proposals are being registered... Do you want to close registration ?', 'info');
            }

            if (currentSession.item.status === VotingSessionWorkflowStatus.ProposalsRegistrationEnded) {
                return renderAlert('You will start the voting session. Do you want to continue ?', 'warning');
            }

            if (currentSession.item.status === VotingSessionWorkflowStatus.VotingSessionStarted) {
                return renderAlert(`There are still ${nbVotesRemaining} voters to vote`, 'info');
            }
            ...
            if (!voters.items[account]) {
                return renderAlert('You are not registered to participate to this voting session', 'error');
            }

            if (currentSession.item.status === VotingSessionWorkflowStatus.ProposalsRegistrationStarted) {
                const nbAlreadyProposed = voters.items[account]?.nbProposals;
                return nbAlreadyProposed < MAX_PROPOSALS ?
                    renderAlert(`Feel free to add new proposals... (remains ${MAX_PROPOSALS - nbAlreadyProposed})`, 'warning') :
                    renderAlert('You already proposed 3 proposals which is the maximum', 'success');
            }
    ...
    return (
        <>
            <Grid container justifyContent={'space-between'} alignItems={'center'}>
                ...
                <Grid item>
                    {currentSession.item.$capabilities.$canDelete && <Tooltip placement={'bottom'} title={'Delete new session'}>
                            <IconButton color={'primary'} onClick={() => setDeleteDialogVisible(!deleteDialogVisible)}>
                                <AddCircleIcon/>
                            </IconButton>
                        </Tooltip>}
                    <Grid item>
