...
export async function getContractInfo(web3: Web3, importsDefinition: IDynamicContractImportDefinitions, contractName: string): Promise<IContractInfo> {
...
export function findRpcMessage(error: Error): string {
    ...
    return rpcMsg?.value.data.message.replace('VM Exception while processing transaction: revert ', '') || error.message;
}
...
export function useNetwork(account: string | undefined): void {
    ...
    useEffect(() => {
        ...
        provider.on("accountsChanged", async (accounts: string[]) => {
            ...
            // force get accountaddress as accounts[0] does not have correct case
            const account = (await web3.eth.requestAccounts())[0];
            console.log("account", account)
            dispatch(SET_CURRENT_ACCOUNT(account))
        });
